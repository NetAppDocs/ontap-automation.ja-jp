...
.PowerShellツールキット
WindowsホストからONTAPクラスタの管理を自動化するには、NetApp ONTAP PowerShell Toolkitを使用できます。詳細については、link:../pstk/learn-about-pstk.html["NetApp ONTAP PowerShell ツールキットについて"]を参照してください。
...
.NetApp Console自動化ハブ
その https://console.netapp.com/automationHub["NetApp Console自動化ハブ"^] コンソールの Web ユーザー インターフェイスから利用できます。自動化ハブは、 ONTAPと他の製品の導入および統合を自動化するのに役立つパッケージ化されたソリューションへのアクセスを提供します。見る https://docs.netapp.com/us-en/netapp-automation/["NetApp自動化"^] ドキュメントと詳細については、こちらをご覧ください。
